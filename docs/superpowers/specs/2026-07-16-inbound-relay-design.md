...
- `commandHandler.js:250-251` — the dedup claim is synchronous *before any `await`* **by design**, so exactly one member-bot handles a group command. Any `await` inserted above it breaks that invariant. The relay hook must not do this.
